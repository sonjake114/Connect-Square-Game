...
class PuzzleGenerator implements PuzzleSource {
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            int nFound;
            nFound = 0;
            PlaceList potentialsuc = new PlaceList();
            if (sq.successor() == null && sq.direction() != 0
                    && sq.sequenceNum() != w * h) {
                for (Place temp : sq.successors()) {
                    if (sq.connectable(model.get(temp))) {
                        potentialsuc.add(temp);
                        found = model.get(temp);
                        nFound += 1;
                ...
                if (sq.sequenceNum() > 0) {
                    for (Place suc : potentialsuc) {
                        if (model.get(suc).sequenceNum() > 0) {
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                PlaceList potentialpred = new PlaceList();
                for (Place temp: sq.predecessors()) {
                    if (model.get(temp).connectable(sq)) {
                        potentialpred.add(temp);
                        found = model.get(temp);
                        nFound += 1;
                    }
                }
                ...
                if (sq.sequenceNum() > 0) {
                    for (Place temp1 : potentialpred) {
                        if (model.get(temp1).sequenceNum() > 0
                                && model.get(temp1).connectable(sq)) {
                            nFound = 1;
                            found = model.get(temp1);
                        }
                    }
                }
